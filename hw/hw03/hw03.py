...
def missing_digits(n):
    ...
    if n < 10:
        ...
        return missing_digits(n//10) + miss(n % 10 - (n // 10) % 10)
...
def count_coins(change):
    ...
    True
    """
    def inner_cout(change, max = 25):
        if change < 0:
            return 0
        if change == 0:
            return 1
        if change < max:
            return inner_cout(change, descending_coin(max))
        if change == 1:
            if max != 1:
                return 0
            else:
        ...
        if max == 1:
            return 1
        return inner_cout(change - max, max) + inner_cout(change, descending_coin(max))
    return inner_cout(change)
