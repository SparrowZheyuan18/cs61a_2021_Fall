...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new_paragraph = [paragraph for paragraph in paragraphs if select(paragraph)]
    if len(new_paragraph) <= k:
        return ''
    else:
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def parabout(para):
        flag = [word for word in topic if word in remove_punctuation(para).lower().split()]
        if flag == []:
            return False
        else:
            return True
...
def accuracy(typed, reference):
    ...
    if typed_words == [] or reference_words == []:
        return 0/1
    return len([typed_words[i] for i in range(0, min(len(typed_words), len(reference_words))) if typed_words[i] == reference_words[i]])/len(typed_words)*100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    low_diff = min([diff_function(typed_word, valid_word, limit) for valid_word in valid_words])
    if low_diff > limit:
        return typed_word
    else:
        for i in range(0, len(valid_words)):
            if diff_function(typed_word, valid_words[i], limit) == low_diff:
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [[times_per_player[j][i] - times_per_player[j][i-1] for i in range(1, len(times_per_player[j]))] for j in range(0, len(times_per_player))]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    ans_list = [[] for i in player_indices]
